...
class API extends REST {
    ...
    private function _sign(){
        ...
        if(!empty($this->_request) ){
            try {
                   ...
                   $res = $this->db->regUser($json_array);
                   if ( $res ) {
                   $result = array('return'=>'Zarejestrowano nowego użytkownika!');
                   $this->response($this->json($result), 200);
                     } else {
                        $result = array('return'=>'Istnieje już konto założone na podany adres email!');
                        $this->response($this->json($result), 200);
                     }
            } catch (Exception $e) {
                $this->response('', 400) ;
    ...
    private function _log(){
        ...
        if(!empty($this->_request) ){
            try {
                   $json_array = json_decode($this->_request,true);
                   $res = $this->db->logUser($json_array);
                   if ( $res ) {
                    session_start() ;
                    $_SESSION['auth'] = 'OK';
                    $_SESSION['user'] = $json_array[email];
                   $result = array('return'=>'Zaktualizowaliśmy bazę danych o Twoje odpowiedzi!');
                   $this->response($this->json($result), 200);
                     } else {
                        $result = array('return'=>'Błędne dane logowania!');
                        $this->response($this->json($result), 200);
                     }
            } catch (Exception $e) {
                $this->response('', 400) ;
